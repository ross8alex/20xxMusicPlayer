...
public class PlaylistAdapter extends RecyclerView.Adapter {
    ...
    public PlaylistAdapter(List<Playlist> data, Context activity, AlbumClickListener listener){
        items = data;
        context = activity;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.group_item_layout, parent, false);

        PlaylistAdapterViewHolder vh = new PlaylistAdapterViewHolder(itemView, albumClickListener);
        return vh;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        PlaylistAdapterViewHolder viewHolder = (PlaylistAdapterViewHolder)holder;
        viewHolder.GenreName.setText(item.GetName());
        viewHolder.NumberOfSongs.setText("");
        viewHolder.NumberOfAlbums.setText("");
        ...
        List<PlaylistSong> distinctSongs = item.PlaylistSongs.stream().filter(distinctByKey(PlaylistSong::GetArt)).collect(Collectors.toList());
        viewHolder.NumberOfSongs.setText(String.valueOf(item.PlaylistSongs.size()) + " song" + ((item.PlaylistSongs.size() != 1) ? "s":""));
...
class PlaylistAdapterViewHolder extends RecyclerView.ViewHolder{
    ...
    public PlaylistAdapterViewHolder(@NonNull View itemView, AlbumClickListener listener) {
        super(itemView);
        ...
        itemView.setOnClickListener(v -> listener.onItemViewClicked(getAdapterPosition(), AdapterType.PLAYLIST));
        PlayButton.setOnClickListener(v -> listener.onPlayButtonClicked(getAdapterPosition(), AdapterType.PLAYLIST));
        Overflow.setOnClickListener(v -> listener.onOverflowClicked(getAdapterPosition(), itemView, AdapterType.PLAYLIST));
    }
}
